...
export const generatePdf = async (htmlContent: string, documentId: string) => {
  ...
  const pdfBuffer = await page.pdf({ format: "A4", printBackground: true, footerTemplate: '<div style="font-size: 10px; text-align: center;">Your desired footer string here</div>', });
  await browser.close();
  return pdfBuffer;
};